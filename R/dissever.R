...
.dissever <- function(
  ...
  for (k in 1:max_iter){
    ...
    if( ! grepl('^gwr',method) ) {
      ...
      dMat <- gw.dist(dp.locat=as.matrix(data.frame(lat, lon)), rp.locat=as.matrix(data.frame(lat_spl, lon_spl)), focus=5, longlat=TRUE)
      if (verbose) print(dMat)
      baux <- bw.gwr(form, data = datagwr, kernel="gaussian", longlat=TRUE, adaptive=TRUE, dMat=NULL )
      if (verbose) message('| -- updating model')
