...
.update_model <- function(vars, y, method = 'rf', control, tune_grid, data_type="numeric", latLong=NULL) {
  ...
  if ( method == 'gwrm' ) { 
    ...
    fit <- lme( fixed=as.formula("x ~ . - dummy") , data=data.frame( vars , latLong, x=y_aux , dummy=rep.int( 1 , length(y_aux) ) ) , random = ~ 1 | dummy, method = "ML" , correlation = corGaus(form = ~ lat+long | dummy ) )
...
.bootstrap_ci <- function(fit, fine_df, level = 0.9, n = 50L, data_type="numeric", latLong=NULL ) {
  ...
  boot_samples <- boot(df, function(data, idx, method = reg_method) {
    ...
    else if ( method == 'lme' ) bootstrap_fit <- lme( fixed=as.formula(".outcome ~ . - dummy") , data=data.frame( bootstrap_df , latLong[idx, ], dummy=rep.int( 1 , nrow(bootstrap_df) ) ) , random = ~ 1 | dummy, method = "ML" , correlation = corGaus(form = ~ lat+long | dummy ) )
